...
Modèle LLM :
  - Ollama Cloud : minimax-m2.5:cloud (via OLLAMA_CLOUD_API_KEY)
  - Endpoint     : https://cloud.ollama.com  (ou OLLAMA_CLOUD_HOST)
  - Fallback     : Hugging Face Inference API si OLLAMA_CLOUD_API_KEY absent
...
Environment variables:
    ...
    OLLAMA_CLOUD_HOST (optionnel, défaut: https://cloud.ollama.com)
    OLLAMA_CLOUD_MODEL (optionnel, défaut: minimax-m2.5:cloud)
    HF_API_TOKEN                                    # Fallback si Ollama Cloud absent
"""
...
SITE_URL = os.getenv("SITE_URL", "https://mygoodpick.com").rstrip("/")
REVALIDATE_SECRET = os.getenv("REVALIDATE_SECRET", "")
HF_API_TOKEN = os.getenv("HF_API_TOKEN", "")

# ── Ollama Cloud config ───────────────────────────────────────────────────────
OLLAMA_CLOUD_API_KEY = os.getenv("OLLAMA_CLOUD_API_KEY", "")
OLLAMA_CLOUD_HOST = os.getenv("OLLAMA_CLOUD_HOST", "https://api.ollama.com").rstrip("/")
...
def _call_ollama_cloud(prompt: str, model: Optional[str] = None) -> str:
    """
    Call Ollama Cloud API (native Ollama /api/chat endpoint).
    See : https://api.ollama.com
    """
    target_model = model or OLLAMA_CLOUD_MODEL
...
def _call_hf(prompt: str) -> str:
    ...
    if not HF_API_TOKEN:
        raise RuntimeError(
            "OLLAMA_CLOUD_API_KEY et HF_API_TOKEN tous les deux absents. "
            "Configure au moins l'un des deux dans .env."
        )
...
def call_llm(prompt: str, model: Optional[str] = None) -> str:
    """Call Ollama Cloud (minimax-m2.5:cloud). Fallback vers HF si clé absente."""
    if OLLAMA_CLOUD_API_KEY:
        target = model or OLLAMA_CLOUD_MODEL
        print(f"  🤖 Ollama Cloud ({target}) …")
        return _call_ollama_cloud(prompt, target)
    else:
        print(f"  ⚠  OLLAMA_CLOUD_API_KEY absent → fallback HF ({HF_TEXT_MODEL}) …")
        return _call_hf(prompt)
